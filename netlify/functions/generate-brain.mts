...
export default async (req: Request, context: Context) => {
  ...
  const { dopamine, serotonin, substance } = await req.json();
  ...
  try {
    ...
    return Response.json({ error: "No image data generated" }, { status: 500 });
  } catch (error) {
    console.error("Error generating brain state image:", error);
    return Response.json(
      { error: "Failed to generate brain image" },
      { status: 500 }
    );
  }
};
